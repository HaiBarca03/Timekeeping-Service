...
export class ShiftResolverService {
  ...
  async resolveShift(context: CalculationContext): Promise<ShiftContext> {
    ...
    if (holiday) {
        ...
        const isMaternity = context.leaveHours > 0 && context['leaveTypeCode'] === 'MATERNITY';
        ...
        if (isMaternity) {
            ...
            if (isEnjoyable) {
                ...
                if (hasAssignment) {
                    ...
                    this.logger.debug(`Lễ trùng ngày nghỉ/ngày không gán ca -> Không hưởng công lễ.`);
                }
            }
        }
    }
    ...
    if (dayOfWeek === 6 && isOfficeGroup && employee.is_saturday_off) {
        if (!hasPunches) {
            ...
            if (offCount < maxOffDays) {
                this.logger.debug(`Thứ 7 nghỉ chế độ (Không đi làm): ${offCount + 1}/${maxOffDays}`);
                return this.markAsConfiguredOff(context);
            }
        } else {
    ...
    return isStoreGroup ? this.resolveStoreShifts(context) : this.resolveOfficeShift(context);
  }
  ...
  private async countConfiguredOffDays(employeeId: string, date: Date): Promise<number> {
    const startOfMonth = new Date(date.getFullYear(), date.getMonth(), 1);
    const calculationDateStart = new Date(date.getFullYear(), date.getMonth(), date.getDate());

    return await this.timesheetRepo.count({
      where: {
        employee_id: employeeId,
        attendance_date: Between(startOfMonth, Raw(alias => `${alias} < :target`, { target: calculationDateStart.toISOString() })),
  ...
  private async checkHasAssignment(context: CalculationContext): Promise<boolean> {
    const { employee, date } = context;
    const groupCode = employee.attendanceGroup?.code;
