...
export class LeaveStrategy {
  ...
  async process(context: CalculationContext): Promise<void> {
    ...
    const leaveRequests = await this.requestRepo.createQueryBuilder('request')
    ...
    for (const req of leaveRequests) {
      ...
      const startDate = new Date(new Date(detail.start_time).toISOString().split('T')[0]);
      
      // Tính ngày thứ bao nhiêu của kỳ nghỉ
      const dayIndex = Math.floor((currentDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)) + 1;

      // --- RULE: ĐỐI TƯỢNG (SRS: Nhân viên chính thức của 4 công ty) ---
      if (!isOfficial && lt.code !== 'UNPAID_LEAVE') {
        this.logger.warn(`NV ${employee.id} không phải chính thức, loại bỏ nghỉ có lương/chế độ.`);
        continue;
      }
      ...
      if (lt.code === 'MATERNITY' || lt.code === 'THAI_SAN') {
        ...
        const diffDays = (appliedDate.getTime() - createdAt.getTime()) / (1000 * 3600 * 24);
        if (diffDays < 30) {
           this.logger.error(`Đơn thai sản ${req.id} vi phạm quy tắc nộp trước 1 tháng (Chỉ nộp trước ${Math.floor(diffDays)} ngày)`);
      ...
      if (policyLimits[lt.code] && dayIndex > policyLimits[lt.code]) {
        this.logger.warn(`Ngày nghỉ thứ ${dayIndex} của loại ${lt.code} vượt giới hạn cho phép. Không tính công.`);
      ...
      if (lt.isDeductLeave) {
        const rule = await this.policyRuleRepo.findOne({
          where: { 
            policyId: (employee as any).leavePolicyId || employee.leavePolicy?.id,
            leaveTypeId: lt.id 
          }
        });
        ...
        if (!rule || Number(rule.quotaDays) <= 0) {
          this.logger.error(`NV ${employee.id} hết quota cho loại ${lt.code}. Không tính công nghỉ.`);
          currentRequestHours = 0;
        }
      }
    ...
    if (dayTotalLeaveHours > 0 && mainLeaveType) {
      ...
      this.logger.debug(`Final Leave: ${context.leaveHours}h (${context.leaveValue} công) cho ${employee.id}`);
    }
  }
}