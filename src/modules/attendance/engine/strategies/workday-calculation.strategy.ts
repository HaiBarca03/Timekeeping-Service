...
export class WorkdayCalculationStrategy {
  ...
  process(context: CalculationContext): void {
    ...
    if (context.isConfiguredOffDay || context.isHoliday) {
      ...
      this.logger.debug(`OffDay/Holiday Detected: Workday = ${context.finalActualWorkday}`);
      return;
    }
    ...
    for (const punch of context.punches) {
        ...
        if (!isFactoryGroup) {
            if (punch.miss_check_in || punch.miss_check_out || punch['is_invalid_workday']) {
                this.logger.warn(`Punch rejected for Office/Store: Violation detected, 0 hours assigned.`);
    ...
    const totalPenalty = (context.latePenalty || 0) + (context.earlyPenalty || 0);
